...
/**
 * Repay token into Alphalend
 * @param agent - SuiAgentKit instance
 * @param params - IRepayParams
 * @returns Promise resolving to the transaction hash
 */
export async function repay_alphalend(
  agent: SuiAgentKit,
  params: IRepayParams,
  ...
  try {
    ...
    logger.error(error);
    throw new Error(`Failed to repay token from Alphalend: ${error.message}`);
  }
}
...
const getTransactionPayload = async (
  ...
  try {
    ...
    const transaction: any = await alphalendClient.borrow(repayParams);
